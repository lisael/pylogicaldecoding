...
class pyld_build_ext(build_ext):
...
if have_pydatetime and (use_pydatetime or not have_mxdatetime):
    define_macros.append(('PYLD_DEFAULT_PYDATETIME', '1'))
elif have_mxdatetime:
    define_macros.append(('PYLD_DEFAULT_MXDATETIME', '1'))
else:
    error_message = """\
...
if version_flags:
    PYLD_VERSION_EX = PYLD_VERSION + " (%s)" % ' '.join(version_flags)
else:
...
setup(name="pslogicaldecoding",
      version=PYLD_VERSION,
      maintainer="Bruno Dupuis",
      maintainer_email="bd@lisael.org",
      ...
      package_dir={'logicaldecoding': 'lib', 'logicaldecoding.tests': 'tests'},
      packages=['logicaldecoding', 'logicaldecoding.tests'],
      cmdclass={
          'build_ext': pyld_build_ext,
          'build_py': build_py, },
      ext_modules=ext)
