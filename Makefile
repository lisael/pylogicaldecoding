...
SOURCE_C := $(wildcard logicladecoding/*.c logicaldecoding/*.h)
...
PACKAGE := $(BUILD_DIR)/logicaldecoding
PLATLIB := $(PACKAGE)/_logicaldecoding.so
...
VERSION := $(shell grep PYLD_VERSION setup.py | head -1 | sed -e "s/.*'\(.*\)'/\1/")
SDIST := dist/pylogicaldecoding-$(VERSION).tar.gz
